...
export default {
  ...
  methods: {
    ...
    formatTime: function(d) {
      let date = new Date(d);
      return this.dateFormat("YYYY-mm-dd HH:MM", date);
      // return date;
    },
  ...
  watch: {
    ...
    value2: function() {
      ...
      this.loading=true;
      if (this.value2 != null) {
        this.requsetData.begin = this.value2[0];
        this.requsetData.end = this.value2[1];
      } else {
        this.requsetData.begin = undefined;
        this.requsetData.end = undefined;
      }
      console.log(that.requsetData);
      this.$axios.recordList(this.requestData).then(res => {
        if (res.code == 200) {
          for (let i in res.data.content) {
            //roomNum
          ...
          that.requsetData = res.data.content;
          that.totalPages = res.data.totalPages;
        }
        that.loading = false;
        console.log(res);
