  ...
  <div v-loading="loading">
    ...
    <div style="width: 95%;margin-left: 2.5%;margin-top: 30px;border-top: 1px solid #EBEEF5;">
      ...
      >
        ...
        <el-table-column prop="status" label="健康状态" width="150" sortable></el-table-column>
...
export default {
  data() {
    return {
      tableData: [],
      // REGIDENT(住户),PROSECUTER(检查人员),MIGRANTS(外来人员)
      options: [
        {
          value: "",
          label: "全部"
  ...
  methods: {
    ...
    deleteRow: function(e) {
      var that = this;
      console.log(e);
      this.$confirm("锁定" + that.tableData[e].name + ", 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          that.$axios.lock(data).then(res => {
            if (res.code == 200) {
              that.tableData[e].isLock = !that.tableData[e].isLock;
              if (
                that.tableData[e].isLock == true ||
                that.tableData[e].isLock == "1"
              ) {
                that.tableData[e].lock = "已锁定";
              } else if (
                that.tableData[e].isLock == false ||
                that.tableData[e].isLock == "0"
              ) {
                that.tableData[e].lock = "正常";
              }
              that.loading = false;
              this.$message({
                type: "success",
    ...
    change(e) {
      ...
      console.log(e);
      this.$confirm(
        "更改" + that.tableData[e].name + "检查权限, 是否继续?",
        "提示",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
        ...
        .then(() => {
          ...
          that.$axios.persecuter(data).then(res => {
            if (res.code == 200) {
              that.loading = false;
              this.$message({
                type: "success",
        ...
        .catch(() => {
          that.tableData[e].isProsecutor = !that.tableData[e].isProsecutor;
        });
    }
  },
  created() {
